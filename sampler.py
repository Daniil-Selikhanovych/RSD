...
class BaseSampler:
    ...
    def  _add_noise(self, model):
        self.write_log(f"Expand input kernel for noise in the student model")
        ...
        final_conv.weight.data = torch.cat([model.input_blocks[0][0].weight.data,
                                            noise_conv.weight.data.to(model.input_blocks[0][0].weight.data.device)],
                                           dim=1)
...
class Sampler(BaseSampler):    
    def sample_func(self, y0, noise_repeat=False, one_step=False, apply_decoder=True):
        ...
        if self.configs.model.add_noise:
            model_kwargs['noise_input'] = torch.randn(size=y0.shape[:1] + (self.configs.model.noise_channels, ) + y0.shape[2:],
                                                      device=y0.device)  
        if not self.ddim:        
        ...
        if bs > 1:
            ...
            if not in_path.is_dir():
                ...
                for im_path in tqdm(im_path_list):
                    ...
                    extension = os.path.basename(im_path).split(".")[-1]

                    im_path = out_path / f"{im_path.stem}.{extension}"
                    util_image.imwrite(im_sr, im_path, chn='bgr', dtype_in='uint8')
                    if return_tensor:
                        return_res[im_path.stem]=im_sr_tensor
